...
def make_vocab(max_len=4000, append_bos=False, append_eos=True, data_dir=DATA_DIR, save=True):
    ...
    l_max = max_len - int(append_bos) - int(append_eos)

    # decode to the right format
    decode = lambda x: ast.literal_eval(x).decode("utf-8")[:l_max] 
...
def process_raw_data(
    ...
    """
    assert kind in ['train', 'val', 'test'], "kind must be one of 'train', 'val' or 'test', not {}".format(kind)
    ...
    decode = lambda x: ast.literal_eval(x).decode("utf-8")[:max_len]
    df['text1'] = df['text1'].apply(decode)
    df['text2'] = df['text2'].apply(decode)
    lengths1 = torch.tensor(df['text1'].apply(len))
    lengths2 = torch.tensor(df['text2'].apply(len))
    ...
    def numericalize(example):
        ...
        if len(indices) < max_len:
            indices = utils.pad_sequence(indices, max_len, pad_val=vocab["<pad>"])
    ...
    if kind == 'train':
        ...
        for i in range(num_files):
            ...
            text1 = torch.from_numpy(np.stack(text1, axis=0))
            text2 = [numericalize(tokens) for tokens in df_chunk['text2']]
            text2 = torch.from_numpy(np.stack(text2, axis=0))
            labels = targets[start_idx:end_idx]
            l1 = lengths1[start_idx:end_idx]
            l2 = lengths2[start_idx:end_idx]
            ds = TensorDataset(text1, text2, labels, l1, l2)
            torch.save(ds, f'{data_dir}/aan_{kind}_{i}.pt')
            print(f"Saved {data_dir}/aan_{kind}_{i}.pt with {len(ds)} samples.")
        ...
        text2 = torch.from_numpy(np.stack(text2, axis=0))
        ds = TensorDataset(text1, text2, targets, lengths1, lengths2)
...
if __name__ == "__main__":
    ...
    process_raw_data(vocab, max_len=4000, kind='test')
    process_raw_data(vocab, max_len=4000, kind='val')
    #AAN()
